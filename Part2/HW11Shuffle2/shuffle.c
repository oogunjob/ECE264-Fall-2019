...
void helperFunction(CardDeck combinedDeck, CardDeck leftDeck, CardDeck rightDeck, int round);
void reShuffle(CardDeck deck, int num);
...
void interleave(CardDeck leftDeck, CardDeck rightDeck, int round)
{
    ...
    helperFunction(tempDeck, leftDeck, rightDeck, round);
...
void helperFunction(CardDeck combinedDeck, CardDeck leftDeck, CardDeck rightDeck, int round)
{
	...
	if (leftDeck.size == 0)
    {
        ...
        // reshuffles the deck
        reShuffle(combinedDeck, round - 1);
    ...
    helperFunction(combinedDeckA, new_leftDeck, rightDeck, round);
    ...
    // calls the function again for the combined deck
    helperFunction(combinedDeckB, leftDeck, new_rightDeck, round);
}
...
void shuffle(CardDeck origDeck, int round)
{
  ...
  if(numCombinations > 0)
  {
    ...
    {
      interleave(leftDeck[count], rightDeck[count], round);
    }
